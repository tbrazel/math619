...
We refer the reader to the following excellent sources for the basics of category theory: \cite{WOMP},\cite{Torres-cat}, \cite{Riehl-context}. We skip the definition of category but recall a list of standard examples of categories that you have certainly encountered  before. 
...
$\Grp$ & groups &group homomorphisms \\\hline
	 ...
	 $\1$ & single object & only identity arrow\\\hline
	 $\2$ & two objects, denoted 0 and 1 & single non-identity morphism $0\to 1$.\\
	 \bottomrule
    \end{tabular}
\end{table}
...
\begin{customenvironment}{Examples of opposite categories} The construction $\mathscr{C}^\op$ occurs frequently in mathematics. Some interesting examples follow:
\begin{itemize}\itemsep0em 
    \item the category of affine schemes is equivalent to $\Ring^\op$
    \item the category $\Top^\op$ is, in some sense, equivalent to the category of frames \cite{top-op-mathse}
    \item the opposite category of finite sets is equivalent to the category of finite Boolean algebras
    \item for any category $\mathscr{C}$, we have that $(\mathscr{C}^\op)^\op$ is just $\mathscr{C}$
\end{itemize}
...
\begin{exercise} Show that the data of a natural transformation is the same as a functor $H : \C\times\2 \to \D$, such that $H(-,0)=F$ and $H(-,1) = G$.
\end{exercise}
...
The Yoneda lemma says that the set of natural transformations$\Hom_\C(-,c)\Rightarrow F$ is in bijection with $F(c)$, naturallyin $c$.
...
\begin{definition} Given a pair of functors $F: \C \leftrightarrows \D:G$, we say they are \textit{adjoint} if there exist natural transformations $\eta : \id_\C \Rightarrow GF$ (called the \textit{unit}) and $\epsilon : FG \Rightarrow \id_\D$ (called the \textit{counit}) satisfying the triangle identities:
...
\begin{definition} Given a pair of functors $F: \C \leftrightarrows \D:G$, we say they are \textit{adjoint} if, for each $a\in \C$, $b\in\D$, there exists an isomorphism
\begin{align*}
...
\begin{exercise} Demonstrate the equivalence of these definitions.
\end{exercise}
...
\begin{definition} A functor $F: \C \to \D$ is an \textit{equivalence of categories} if there exists $G : \D \to \C$ such that $FG \cong \id_\D$ and $GF\cong \id_\C$. We say that $F$ is an \textit{isomorphism of categories} if $FG = \id_\D$ and $GF = \id_\C$.
\end{definition}

\begin{proposition} $F: \C \to \D$ is an equivalence of categories if and only if $F$ is:
\begin{itemize}\itemsep0em
...
%
%
%\textbf{Notation}: Let $Y^X$ or $\Map(X,Y)$ denote the \textit{mapping space} between two topological spaces, whose underlying set is the collection of continuous maps $X\to Y$, and which is equipped with the \textit{compact-open topology}, generated by the subbasis
%\begin{align*}
...
%\end{align}
%which is always a homeomorphism onto its image, but it is not always surjective. Thus $\Map(Y,-)$ and $-\times Y$ are not adjoint. In order to turn Equation \ref{eqn:mapping-space-adjunction} into a homeomorphism,
%%%%
%% LECTURE 3
...
%\end{exercise}
%
%Let $kX$ denote the set $X$ equipped with the $k$-closed topology. Then the identity map $kX \to X$ is continuous.
%
...
%\end{definition}
%
%\begin{customenvironment}{Examples of CG spaces} CW complexes, metric spaces.
%\end{customenvironment}
%
%\begin{proposition} If $X$ is a compactly generated space, and $Y$ is any space, then $f: X \to Y$ is continuous if and only if $f: X \to kY$ is continuous.
%\end{proposition}
%
%\begin{customenvironment}{Upshot} For the functors $i:\CG \leftrightarrows \Top:k$, we have that
%\[
...
%\end{theorem}
%
%This category is good enough for most applications, but to make things a little better, we impose an additional separation axiom.
%
...
%\end{definition}
%
%\begin{customenvironment}{Examples of weak Hausdorff spaces} CW complexes, metric spaces, etc.
%\end{customenvironment}
%
...
Before moving on to many examples, we state a proposition whose usefulness is hard to underestimate. It says that left adjoints preserve colimits and right adjoints preserve limits.
...
\begin{align*}
	\lim_I F = F(0) \prod F(1) \\
	\colim_I F = F(0) \coprod F(1).
\end{align*}
\end{definition}

\begin{customenvironment}{Examples of products and coproducts} $\ $
\begin{itemize}\itemsep0em
...
%\end{note}
%
%\begin{customenvironment}{Issue} The product of two CW complexes $X\times Y$ is only a CW complex if the product topology is the same as the weak topology. We also note that the product topology is generally not as fine as the weak topology.
%\end{customenvironment}
%
%\begin{customenvironment}{Partial results}$\ $
%\begin{enumerate}
...
%
%
%But the good news is that in $\CGWH$ we have that $X\times_k Y$ is always a CW complex (proof in \cite{Hatcher}).
...
%\end{proof}
%
%We want $\CGWH$ to be complete and cocomplete.
%
%
%
%\textbf{Recall}: We had adjunctions
%\begin{align*}
...
%
%	For limits, we claim that $\lim_J G$ for $J\to \CG$ is $k\lim_J (i\circ G)$ in $\CG$.
%	\begin{proof}
%	\begin{align*}
%		\Hom_\CG (X, k\lim_J (i\circ G)) \cong \Hom_\Top (iX, \lim_J i \circ G).
%	\end{align*}
%
%
%	\textbf{Side remark}: We may check that, in any $\C$ which has $J$-shaped limits,
%	\begin{align*}
%		\Hom_\C (Z, \lim_J F) \cong \Hom_{\C^J} (\Delta \circ Z, F)
%	\end{align*}
%	where $\Delta : \C \to \C^J$ is the diagonal.
%
%	Using this remark above, we have that
%	\begin{align*}
%		\Hom_\Top (iX, \lim_J i \circ G) &\cong \Hom_{\Top^J} (\Delta \circ iX, iG) \\
%		&\cong \Hom_{\Top^J} (i\circ \Delta X, iG) \\
%		&\cong \Hom_{\CG^J} (\Delta X, k\circ iG) \\
%		&\cong \Hom_{\CG^J}(\Delta X, G),
%	\end{align*}
%	where this last isomorphism is since $G$ already takes values in $\CG$. Finally, we see that
%	\begin{align*}
%		\Hom_{\CG^J}(\Delta X, G) &\cong \Hom_\CG (X, \lim_J G).
%	\end{align*}
%
%	Since this is true for all objects $X$, we may check that $k\lim_J (i\circ G) = \lim_J G$.
%
%	\end{proof}
%
%	\textbf{Upshot}: We have that $\CG$ is complete and cocomplete and we know how to complete limits and colimits:
%	\begin{itemize}
...
%
%\begin{customenvironment}{Notation} We will redefine the category of topological spaces to be $\Top := \CGWH$, and redefine $\times := \times_k$, $\Map := k\Map$, etc
%\end{customenvironment}
%
%Now we have that $\Top_\ast$ is the category of based (CGWH) spaces and based maps. We therefore have $\Map_\ast (X, Y)$ which is the space of based maps, and we note that we may base this mapping space at the constant map $c_\ast$ sending every $x\in X$ to the basepoint $y_0$ of $Y$.
%
...
%\end{lemma}
%
%\begin{proof}[Proof idea] For (1), we need $\CGWH$ for product to preserve quotients. We have composite quotients
%\begin{align*}
%	X\times Y \times Z \to (X\smashprod Y)\times Z \to (X\smashprod Y)\smashprod Z.
%\end{align*}
%Since we are in $\CGWH$, the composition of quotient maps is a quotient map, and may be identified with the quotient
%\begin{align*}
...
%
%
%\begin{definition} A \textit{symmetric monoidal category}\index[ind]{symmetric monoidal category} is a category $(\C,\otimes,\1)$ is a category equipped with a functor
%\begin{align*}
%	- \otimes - : \C \times \C \to \C,
%\end{align*}
...
%	\1 \otimes c &\cong c \cong c\otimes \1,
%\end{align*}
...
\begin{definition} A \textit{based homotopy}\index[ind]{homotopy!based} from $f$ to $g$, where $f,g : (X,x_0) \to (Y,y_0)$ is a homotopy $h:f\simeq g$ with the extra condition that each $h(-,t) : (X,x_0) \to (Y,y_0)$ is based.

\textbf{Check}: This is equivalent to a based map $h : X \smashprod I_+ \to Y$.
\end{definition}

\begin{definition} The set of based homotopy equivalence classes of maps is denoted $[X,Y]_\ast$. For example, $[S^1, (X,x_0)]_\ast = \pi_1(X,x_0)$.
\end{definition}
...
\begin{proposition} The functor $\gamma : \Top \to \Ho(\Top)$ is universal among functors $F: \Top \to \C$ which send homotopy equivalences into isomorphisms.\footnote{This is equivalent to being homotopy invariant, meaning $F(f) = F(g)$ if $f\simeq g$.} That is, for any such functor, it factors uniquely as
\[
	\begin{tikzcd}
...
To see that this is well-defined, suppose $g\simeq f_\alpha$. Then there exists $h: X\times I \to Y$ such that $h \circ i_0 = f_\alpha$ and $f\circ i_1 = g$, where $i_0,i_1 : X \to X\times I$ are the inclusions at time 0 and 1. Let $\pi_X : X\times I \to X$ be the projection on $X$, and note that $i_0, i_1, \pi_X$ are all homotopy equivalences. Therefore $F(i_0), F(i_1), F(\pi_X)$ are all isomorphisms. We note also that
\begin{align*}
...
\begin{customenvironment}{Warning} The categories $\Ho(\Top)$ and $\Ho(\Top_\ast)$ do not have all limits and colimits.
...
For $n=0$, we obtain the classes of based maps $S^0 \to X$, which correspond to path components of $X$. Thus $\pi_0$ is a functor to based sets.
...
For $n\geq 2$, we note that $S^n \cong S^1 \smashprod \cdots \smashprod S^1$ is $n$ copies of the 1-sphere smashed together.
	...
	\end{exercise}
	\begin{proof} Left adjoints preserve colimits, and note that $-\smashprod Z : \Top_\ast \to \Top_\ast$ is a left adjoint.
	\end{proof}

	Thus we can define $n$ different pinch maps:
	\begin{align*}
		p_1 &: S^n \cong S^1 \smashprod S^{\smashprod (n-1)} \xto {p\smashprod \id} (S^1 \vee S^1)\smashprod S^{n-1} = S^n \vee S^n \\
		&\vdots \\
		p_k &: S^n \cong S^{k-1}\smashprod S^1 \smashprod S^{n-k} \xto{\id\smashprod p \smashprod \id} S^{k-1}\smashprod (S^1 \vee S^1)\smashprod S^{n-k} \cong S^n \vee S^n \\
		&\vdots
	\end{align*}
