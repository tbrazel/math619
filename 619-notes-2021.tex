...
(Dual definition using the specifying the counit instead of the unit.)
\end{definition} 
...
Some examples of CG spaces are metric spaces and all locally compact spaces, so in particular, CW complexes.
...
Another reason why we like weak Hausdorff spaces is because they interact nicely with compactly generated spaces. For example,
\vspace{-1em}
...
\begin{proof}[Proof idea] For (1), it is important that we are in $\CGWH$, where products preserve quotients, so that 
\begin{align*}
...
\begin{definition} A \textit{symmetric monoidal category}\index[ind]{symmetric monoidal category} is a category $(\C,\otimes,\mathbbm{1})$ is a category equipped with a functor
\begin{align*}
	- \otimes - : \C \times \C \to \C,
\end{align*}
