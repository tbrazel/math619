...
\begin{example}
The pseudo circle is a finite topological space $X=\{a,b,c,d\}$, with topology corresponding to the partial order $a<c, a<d, b<c, b<d$. It turns out that this space is indistinguishable from the circle when it comes to weak equivalence. Consider the  map $f\colon S^1\to X$ which sends the north pole to $c$, the south pole to $d$, the left hemisphere to $a$ and the right hemisphere to $b$. This map is continuous, and it induces an isomorphism on all homotopy groups. However, there is no inverse homotopy equivalence.
\end{example}
