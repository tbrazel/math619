...
%
%
%Observe that $BG = \coprod_{n=0}^\infty G^n \times \Delta^n / \sim$. It turns out this is precisely the geometric realization of a simplicial set. We go on to defining these concepts next.

%\begin{example}  Bott periodicity, in complex $K$-theory, is the result that $\BU \simeq \Omega^2 \BU$. Thus we define an $\Omega$-(pre)spectrum ${\bf \BU} = \{\BU, \Omega \BU, \BU, \Omega \BU,\ldots\}$, which yields a cohomology theory, called complex topological $K$-theory.
%\end{example}
