...
\title{Math 619 Notes}
\author{Thomas Brazelton and Mona Merling\\ with illustrations by Maxine Calle}
\date{Spring 2023 --- Last modified \today}
...
% To start the preview section indexed at 0
...
\begin{example}\label{ex:sphere-spectrum} There is a \textit{sphere spectrum}, denoted $\mathbb{S}$, whose $n$th space is the $n$-sphere $S^n$. The structure maps are the homeomorphisms $S^1 \smashprod S^n \xto{\cong} S^{n+1}$ given by \autoref{ex:homeomorphism-smashing-spheres}.
\end{example}



\begin{example}\label{ex:suspension-spectrum} Given any based space $X$, we denote by $\Sigma^\infty X$ its \textit{suspension spectrum}, whose $n$th space is $\Sigma^n X$, and whose structure maps are the obvious homeomorphisms $\Sigma \Sigma^n X \xto{\cong} \Sigma^{n+1} X$. As an example, we have that $\mathbb{S} = \Sigma^\infty S^0$.
\end{example}
...
\begin{align*}
	\pi_n(X) &= \colim_k \left( \cdots \to \pi_{n+k}(X_k) \xto{\Sigma} \pi_{n+k+1}(\Sigma X_k) \xto{(\sigma_{k})_{\ast}} \to \pi_{n+k+1}(\Sigma X_{k+1}) \to \cdots \right)
\end{align*}
\end{definition}
...
$\Grp$ & groups &group homomorphisms \\\hline
	 $\Ab$ & abelian groups &group homomorphisms \\\hline
         ...
         $\CRing$ & commutative rings& ring homomorphisms \\\hline
...
\begin{itemize}\itemsep0em 
	...
	\item  homology functors $H_i \colon \Top_\ast \to \Grp$ 
	\item  homotopy group functors  $\pi_i \colon \Top_\ast \to \Grp$ 
	\item cohomology functors  $H^i : \Top^\ast \to \Grp$, which are \textit{contravariant} functors.
\end{itemize}
...
\vspace{-1em}
\begin{itemize}\itemsep0em 
    \item the category of affine schemes is equivalent to $\CRing^\op$
     \item the opposite category of sets is equivalent to a subcategory of Boolean algebras (for a discussion of this, see \cite{set-op-mathse})
...
\begin{definition} We define $\D^\C$ or $\Fun(\C,\D)$ as the \textit{functor category} whose objects are functors $\C\to \D$ and whose morphisms are natural transformations.\footnote{We warn the reader that some authors use the convention that $\mathscr{D}^{\mathscr{C}}$ denotes $\mathscr{D}$-valued presheaves on $\mathscr{C}$, which are functors $\mathscr{C}^\op \to \mathscr{D}$.}
\end{definition}
...
Some examples of weak Hausdorff spaces are again CW complexes, metric spaces, etc. One of the reasons why we work with weak Hausdorff spaces as opposed to Hausdorff spaces is that it behaves well with respect to quotients. We recall that in general the quotient of a Hausdorff space, even by a closed relation, is not necessarily Hausdorff. For example, the quotient of the $C_2$-action on a pair of real lines $\R$ which swaps the points between the lines outside of 0, and fixes $0$ on both lines is the ``line with two origins," which is not Haurdorff. Another example is the quotient of the action of the additive subgroup $\Q$ on $\R$, which is $\R/\Q$, which has the trivial topology so it is not Hausdorff. On the other hand, for any closed equivalence relation on $X$ where $X$ is weak Hausdorff, the quotient is also weak Hausdorff.
...
\begin{theorem}\label{thm:LAPC} (\textit{LAPC and RAPL}\footnote{Left adjoints preserve colimits and right adjoints preserve limits.}) Let $F\adj G$ be adjoint functors. Then $F$ preserves colimits and $G$ preserves limits.
\end{theorem}

\begin{exercise}
Prove \autoref{thm:LAPC}.
...
\[
	...
	\end{tikzcd}
\]
commutes. This is exactly the coproduct of sets that we are familiar with. Dually, the limit $A\times B$ satisfies the dual property that, for any $X$ equipped with maps $X\to A$ and $X\to B$, there is a unique map such that
\[
	\begin{tikzcd}
...
\begin{itemize}\itemsep0em
	...
	\item In $\Top$ the formula for the pushout is the same as in $\Set$, and the pushout becomes a gluing construction. 
%	For example, 
%	\[
%		\begin{tikzcd}
...
\begin{note} The product of CW complexes $X\times Y$ gets a cell structure with $n$-cells corresponding to a $p$-cell of $X$ and a $q$-cell of $Y$ where $p+q=n$. The idea behind this is that
\[
...
For $n\geq 2$, we note that $S^n \cong S^1 \smashprod \cdots \smashprod S^1$ is $n$ copies of the 1-sphere smashed together.
	...
	\begin{prop} $(X\vee Y)\smashprod Z \cong (X\smashprod Z)\vee (Y\smashprod Z)$.
	\end{prop}
	...
	\begin{align*}
		p_1 &\colon S^n \cong S^1 \smashprod S^{\smashprod (n-1)} \xto {p\smashprod \id} (S^1 \vee S^1)\smashprod S^{n-1} \cong S^n \vee S^n \\
		&\vdots \\
...
\end{align*}
is not necessarily the identity. Thus we note that $\pi_n(X,x)$ is defined only up to isomorphism without a choice of basepoint. However if $\pi_1(X,x)$ is trivial, then $\pi_n(X)$ is well-defined without a choice of basepoint.
\end{note}
...
\begin{note} If $X$ and $Y$ are path-connected, it is sufficient to require that $f_\ast \colon \pi_n(X,x) \to \pi_n(Y,f(x))$ is an isomorphism for all $n\geq 1$ and for all $x\in X$.
\end{note}
...
\begin{figure}[H]
  \includegraphics[scale=.15]{pics/gammat.png}
  \centering
...
\[
	\begin{tikzcd}
	X\rar[equal]\dar["i_0" left] & X\dar["i_0" left]\ar[dd, bend left=60, "i" right] \\
	X\amalg X\dar\rar["\id_X \amalg f" below] & X\amalg Y\dar["{(i,j)}" left] \\
	X\times I\rar & Mf\po
	\end{tikzcd}
\]
By Proposition \ref{prop:cof-times-space}, $X\amalg X = X\times \del I \cofto X\times I$ is a cofibration, and the bottom diagram is a pushout, so $(i,j)$ is a cofibration. Precomposing with $i_0$, which is also a cofibration, we obtain that $i$ is a cofibration.
\end{proof}
...
\[
	\begin{tikzcd}
	E^I\ar[rr,"\ev_0"]\ar[dd,"p^I" left] & & E\ar[dd,"p" right] \\
	 & X \ar[ur,"f"]\ar[ul,dashed,"\til{h}"]\ar[dl,"h"] \\
	 B^I \ar[rr,"\ev_0" below] & & B.
	 \end{tikzcd}
\]
...
\[
	\begin{tikzcd}
	E^I \ar[drrr,bend left=10,"\ev_0"]\ar[dddr,bend right=10,"p^I" left]\ar[dr,"k"] & & & \\
	& N_p\ar[dd]\ar[rr] & & E\ar[dd,"p" right] \\
	& & X \ar[ur,"f"]\ar[ul,dashed]\ar[dl,"h"]\ar[uull,dashed,bend left=20] & \\
	 & B^I \ar[rr,"\ev_0" below] & & B,
	 \end{tikzcd}
\]
so we note that we are looking for a section $s\colon N_p \to E^I$ so that we may define the lift $\til{h}:=k\circ s$.
\end{note}

\begin{definition} Let $k\colon E^I \to N_p$ with coordinates $E^I\xto{\ev_0} E$ and $E^I\xto{p^I} B^I$. A map $s: N_p \to E^I$ with $k\circ s = \id$ is called a \textit{path lifting function}. Then $p_0 \circ s(e,\beta)(0)=e$ and $p^I\circ s(e,\beta) = \beta$.
\end{definition}

\begin{proposition} The map $p\colon E \to B$ is a fibration if and only if the natural map $k: E^I \to N_p$ has a section (that is, there is a path lifting function).
\end{proposition}

\begin{remark} If $p\colon E\to B$ is a covering space, then $p$ is a fibration with a unique path lifting function.
\end{remark}

\begin{example} Assume $(X,x_0)$ is well based. Then
\begin{align*}
	\ev_{x_0} \colon \Map(X,Y) &\to Y \\
	f &\mapsto f(x_0)
\end{align*}
is a fibration.
\end{example}
...
\end{align*}
is the identity, which will indicate that the connecting homomorphism $\delta$ is trivial.
\end{note}
...
\begin{definition} Let $(X,x_0) \xto{f} (Y,y_0)$ be a based map. Then $Ff\xto{\pi} X \xto{f} Y$ is called a \textit{fiber sequence}\index[ind]{fiber sequence}, where
\begin{align*}
...
\subsection{Long exact sequences in homology and cohomology} 
	...
	\end{align*}
    for all $k$, and $\til{H}_k(X,A) \cong \til{H}_k (Ci)$. This gives the long exact sequence of the ``good pair'' $(Mi,A)$ \cite[Theorem~2.13]{hatcher}. This coincides up to sign with the sequence obtained by applying $\til{H}_n$ to the iterated cofiber sequence of spaces.
	\end{proof}

	 We may also get the long exact sequence in cohomology via the iterated cofiber sequence as follows:
	...
	\end{align*}
	Moreover, $\til{H}^n(X,\pi) \cong [X,K(\pi,n)]_\ast$, that is, reduced cohomology is represented by Eilenberg-Maclane spaces. Then applying $[-,K(\pi,n)]_\ast$ to the iterated cofiber sequence of spaces, one obtains
	\begin{align*}
...
\begin{theorem}[Whitehead's theorem]\label{thm:Whitehead}\index[ind]{Whitehead's theorem} If $X$ is a CW complex and $Y\xto{e} Z$ is an $n$-equivalence, then $e_\ast : [X,Y] \to [X,Z]$ is bijective if $\dim X < n$ and surjective if $\dim X = n$ (where $n$ could be $\infty$).
\end{theorem}
...
\begin{align*}
	e_\ast : [Y,Y] \to [Y,Z]
\end{align*}
is bijective, and since $e\circ f\circ e$ and $\id_Y$ are both mapped to the class of $e$, we get that $f\circ e \simeq \id_Y$.
\end{proof}
...
\begin{enumerate}
	...
	\end{align*}
	induces a map on the one-point compactification $S^n \hookto S^{n+1}$. Consider the CW structure, which we denote $(S^n)_1$ and which has two cells in each dimension. Let $(S^n)_2$ denote the CW structure with one 0-cell and one $n$-cell. % todo swap these notations
	...
	\item We note that the CW structure on $(S^n)_1$ is compatible with the $C_2$ antipodal action in the sense that it sends $k$-cells to $k$-cells, whereas the CW structure on $(S_n)_2$ is not compatible with the action. Note that the CW structure on $(S^n)_1$  gives the usual CW structure on $\RP^n$ by quotienting antipodally. Moreover, we have from the induced CW structure that the inclusions
	\begin{align*}
...
\begin{lemma}[{\cite[4.10]{hatcher}}] Any map $f\colon D^n \to X^{(n)} \cup e^N$ for $N>n$ is homotopic rel $S^{n-1}$ to a map $f' : D^n \to X^{(n)} \cup e^N$ which misses a point in $e^N$.
\end{lemma}
...
\begin{enumerate}
	...
	\item Given pointwise equivalences $A\simeq A'$, $B\simeq B'$ and $C\simeq C'$,  if $A\to B$ and $A'\to B'$ are cofibrations, we get an equivalence $B\cup_A C \simeq B'\cup_{A'} C'$. This is called the \textit{gluing lemma}.\index[ind]{gluing lemma}
\end{enumerate}
...
\begin{enumerate}
	...
	\begin{align*}
		\hocolim\left( \begin{tikzcd}[ampersand replacement=\&] \emptyset\rar\dar \& X \\ Y \end{tikzcd} \right) \simeq X \amalg Y.
	\end{align*}
...
\begin{definition} The \textit{stable homotopy groups}\index[ind]{stable homotopy group} of $X$ are defined as 
\begin{align*}
...
We consider the following axiom which might or might not hold:
\begin{enumerate}\setcounter{enumi}{4}
	\item \textit{dimension axiom} we have that
	\begin{align*}
		E_q (\ast) = \begin{cases} \pi & q=0 \\ 0 & q\neq 0. \\ \end{cases}
	\end{align*}
	where $\pi$ is an abelian group. 
\end{enumerate}
\end{definition}

\begin{remark} Instead of spaces, one may state analogous axioms for CW complexes. An unreduced homology theory $E_\ast$ on $\Ho(\Top^2)$ determines and is determined by a homology theory $E^\CW_\ast$ on $\Ho(\CW^2)$ with all the axioms \textit{except} axiom 4 (which is no longer necessary by Whitehead's theorem), and where we replace ``excisive triad'' by ``CW triad'' in axiom 2. Given a theory on CW pairs, to get a theory defined on all pairs of spaces, we  may then define
\begin{align*}
	E_q (X,A)= E_q^\CW (\Gamma X, \Gamma A).
\end{align*}
\end{remark}
...
\begin{enumerate}
	...
	\item \textit{additivity}: if $X = \bigvee_i X_i$, then there is an isomorphism
	\begin{align*}
...
\textbf{Claim}: For $A\xcofto{i} X$ a cofibration, we have $E_\ast(X,A) \to E_\ast(X/A, \ast) = \til{E}_\ast (X/A)$ is an isomorphism.
\begin{proof}[Proof of claim] 


%For the inclusion $A\xcofto{i} X$, consider the ``wizard hat'' $Ci$ and the ``top hat'' $Mi$, drawn below:
%\begin{figure}[H]
...
Recall that since $A\xcofto{i} X$ is a cofibration, the mapping cone $Ci$ is homotopy equivalent to $X/A$. We obtain an excisive triad on the cone $Ci$ as follows:
...
To do: Include diagram to show that the map $(X,A) \to (X/A,\ast)$ is an isomorphism on $\til{E}_q$ \cite[p.108]{may} \todo{Finish proof with better pics from Maxine}
\end{proof}
...
\begin{align*}
	\amalg X_i \big/ \amalg \ast = \bigvee_i X_i,
\end{align*}
...
We need to show excision and exactness (see \cite[Ch.~14]{may} for the proofs). For the excision trick, it suffices to work with the theory on CW complexes, and note for a CW triad $(X,A,B)$, we have that
\begin{align*}
	A/A\cap B \to X/B
\end{align*}
...
\textbf{Digression.} Consider the category $\FinSet$ of finite sets, which is equipped with a monoid structure from the operation of set union. In a sense that can be made rigorous (the Barratt-Priddy Theorem), the sphere spectrum is the group completion of $\FinSet$ with respect to set union. This is a homotopical lift on the phenomenon that we witness on $\pi_0$, where $\mathbb{Z}$ is the group completion of $\mathbb{N}$. 
...
There is a link between homology and homotopy groups. First note that from the dimension axiom for a homology theory with $\Z$-coefficients, we can pick a generator $\iota_0$ of $\til{H}_0(S^0)$, and then we can inductively define generators $\iota_n=\Sigma \iota_{n-1}$, using the suspension axiom.

\begin{definition} The \textit{Hurewicz homomorphism}\index[ind]{Hurewicz homomorphism} is the map \begin{align*}
	h : \pi_n(X) \to \til{H}_n(X),
\end{align*}
defined by $h([f])=f_\ast(\iota_n)$.

% TODO perfect pairing definition
\end{definition}

Alternatively, note that a map $S^n \to X$ corresponds to a map $\Delta^n \to X$ where $\del\Delta^n \to \ast$. This corresponds to a cycle in $C_n^{sing}(X)$, which yields an element in $\til{H}_n(X)$.

\begin{theorem} The Hurewicz map is a natural homomorphism which is compatible with suspension \cite[15.1]{may}, \cite[2.A]{hatcher}.
\end{theorem}
...
\begin{align*}
	H_n(X^{(n)}, X^{(n-1)}) \xto{\delta} H_{n-1}(X^{(n-1)}) \to H_{n-1}( X^{(n-1)}, X^{(n-2)}).
\end{align*}
We may check that $d\circ d = 0$.
\end{proof}

\begin{theorem} There is an isomorphism $C_\ast (X) \cong C_\ast^{cell}(X)$ of chain complexes.
\end{theorem}
We see $C_n(X) \cong C_n^{cell}(X)$ by additivity since $X^{(n)}/X^{(n-1)} \simeq \bigvee S^n$, where the wedge is taken over the $n$-cells of $X$. To reconcile the differentials between these chain complexes, we need the Hurewicz homomorphism \cite[15.2]{may}.
...
\begin{enumerate}
	...
	\item \textit{additivity}: if $X = \bigvee_i X_i$, then the maps $X_i \to X$ induce an isomorphism
	\begin{align*}
...
\begin{itemize}
	...
	\item \textit{additivity}: for $X = \bigvee_i X_i$, we have an isomorphism
	\begin{align*}
		[X,Z]_\ast = \left[ \bigvee_i X_i, Z\right]_\ast \xto{\simeq} \prod_i [X_i, Z]_\ast
	\end{align*}
	by the universal properties of products and coproducts.
\end{itemize}

We have seen that this assignment satisfies all the axioms of a cohomology theory except the suspension axiom. 

We can now reformulate \autoref{q:cohom-thy} a little more precisely.
\begin{question}\label{q:cohom-thy2} For what sequences of spaces $\{P_n\}$ are the functors $[-,P_n]_\ast$  a generalized cohomology theory?\end{question}



In order to build a cohomology theory from $[-,P_n]_\ast$, we will need to check when the suspension axiom holds. We will also need to verify that $[X,P_n]_\ast$ is a group, rather than just a set. Recall that we have a natural isomorphism
\begin{align*}
	[\Sigma X, Y]_\ast \cong [X, \Omega Y]_\ast.
\end{align*}
...
\begin{proposition} The set $[X,Z]_\ast$ has the structure of an abelian monoid if $Z$ is a homotopy associative and commutative $H$-space. This set is furthermore an abelian group if $Z$ is \textit{grouplike}\index[ind]{grouplike space}, that is if the multiplication $\mu$ on $Z$ induces a group structure on $\pi_0(Z)$. 

We may impose the stronger condition that we have inverses up to homotopy; that is, we have an ``inversion'' map $\chi : Z\to Z$ such that the diagram commutes up to homotopy:
\[
	\begin{tikzcd}
...
Returning to  \autoref{q:cohom-thy2}, we see that it suffices to pick $P_n$ to be $\Omega^2$ of some other space for each $n$, and we also need that $\til{E}^n(X) = \til{E}^{n+1}(\Sigma X)$, that is,
\begin{align*}
	[X,P_n]_\ast &\cong [\Sigma X, P_{n+1}]_\ast \cong [X,\Omega P_{n+1}]_\ast.
\end{align*}
From this we see it suffices to define the desired sequence by taking a prespectrum $\{P_n\}$ for which the adjoint maps to the structure maps $P_n \xto{\sim} \Omega P_{n+1}$ be weak equivalences.
...
\begin{remark} We have shown that for every $\Omega$-prespectrum $P$, the functor
\begin{align*}
	h_n : \CW_\ast &\to \Ab \\
...
But for any (pre)spectrum $P$, we may replace $P_n$ by $\colim_k \Omega^k P_{n+k}$, which is an $\Omega$-(pre)spectrum. We define the cohomology theory
\begin{align*}
	h^n(X):= \left[X, \colim_k \Omega^k P_{n+k}\right]_\ast.
\end{align*}
\end{note}

In the next section, we will see two examples of $\Omega$-spectra, the Eilenberg MacLane spectrum and the complex $K$-theory spectrum. In order to construct these, we need the notion of classifying space of a group.

\section{Eilenberg MacLane spaces}

For an abelian group $G$, the goal is to define Eilenberg MacLane spaces $K(G,n)$ such that
\[
    \pi_q(K(G,n)) = \begin{cases} G & q=n \\ 0 & \text{otherwise}. \end{cases}
\]



\subsection{The bar construction $BG$}
Let's start with $K(G,1)$, which works for all $G$ (not necessarily abelian). This is also called the \textit{bar construction}, which can also be denoted by $BG$.

\textbf{Idea for the bar construction} The idea for the definition of $BG$ is to take configurations of points in $I$, labeled by elements in $G$, such that
    \begin{itemize}
        \item when points collide, labels combine using multiplication in $G$
        \item points can slide off to infinity.
    \end{itemize} 
    
%    We define $BG$ to be the configuration space of $n+1$ points in the unit interval $I$, over all natural numbers $n$ such that
%\begin{align*}
...
More precisely, the points are configurations of points in $I$, labelled by elements of $G$. Each configuration of $n$ points subdivides the unit interval into $n+1$ segments, so each such configuration determines lengths $t_0, t_1, \dots, t_n$, where $\sum_i t_i = 1$. 

\todo{All these tables are wrong, I had made a mistake. There should be only $n$ not $n+1$ points in a configuration of $n$ points inside $I$. They should be labels on the points, not the intervals. Need to fix and draw new pictures.}
\begin{center}
...
We note that in a configuration of $n$ points labelled by elements of $G$, the points are priori allowed to be on the boundary, or more than a point can be in the same slot in $I$. In other words, for a configuration of $n$ points, the $t_i$ are a priori allowed to be 0. Now we make a bunch of identifications:
...
Observe that $BG = \coprod_{n=0}^\infty G^n \times \Delta^n / \sim$. It turns out this is precisely the geometric realization of a simplicial set. We go on to defining these concepts in the next section.
...
\begin{example} The singular complex of a space $X$. We first define the topological $n$-simplex as
    ...
    \end{align*}
    We may think about  this as viewing an $(n-1)$-simplex as a degenerate $n$-simplex. We have another map for $0\leq i\leq n$
    \begin{align*}
        \sigma_i : \Delta^{n+1} &\to \Delta^{n} \\
        (t_0,\ldots,t_{n+1}) &\mapsto (t_0,\ldots,t_{i-1}, t_i + t_{i+1},\ldots,t_{n+1}),
    \end{align*}
...
\begin{align*}
    ...
    (\Delta^{n} \xto{f} X) &\mapsto (\Delta^{n-1} \xto{\delta_i} \Delta^{n} \xto{f} X),
\end{align*}
and
\begin{align*}
    s_j : \Top(\Delta^{n}, X) &\to \Top(\Delta^{n+1}, X) \\
    (\Delta^{n} \xto{f} X) &\mapsto (\Delta^{n+1} \xto{\sigma_j} \Delta^{n} \xto{f} X).
\end{align*}
\end{definition}
...
\textbf{Intuition} Out of this simplicial set $X_\dot$, we would like to build a topological space which records the combinatorial data of the simplicial set in a topological way. More specifically, we will build a CW complex whose 0-cells are precisely $X_0$, and whose higher cells are non-degenerate simplices, that is, our $n$-cells should not arise as the degeneracy of some lower-dimensional information.
...
\begin{theorem}[{\cite[16.2]{may}}] This morphism $\gamma$ is a weak equivalence. Moreover, $|\Sing(X)|$ is a CW complex with one $n$-cell for each nondegenerate $n$-simplex of $X$.
\end{theorem}
...
For a sketch of the proof, see \cite[Theorem 4.10]{friedman4221elementary}.
...
\begin{exercise} Show that $B(\Z\big/2) \cong \RP^\infty$.
\end{exercise}

\begin{exercise} Show that for the category $\dot \to \dot$, its classifying space is the unit interval $I=[0,1]$.
\end{exercise}

\begin{remark}
We can define simplicial objects in any category, and in particular we can define simplicial spaces and their geometric realization in the same way. Note that if $G$ is a topological group, $BG$ is the geometric realization of a simplicial space $B_\bullet G$, defined in the same way, except now $G$ is a space and not a set throughout the construction. \end{remark}

\begin{example} Let $U$ be the infinite unitary group, as a topological group.  Bott periodicity, in complex $K$-theory, is the result that $\BU \simeq \Omega^2 \BU$. Thus we define an $\Omega$-(pre)spectrum ${\bf \BU} = \{\BU, \Omega \BU, \BU, \Omega \BU,\ldots\}$, which yields a cohomology theory, called complex topological $K$-theory.
\end{example}
...
\begin{exercise} Show that $EG\simeq \ast$ is contractible. 
\end{exercise}
Thus by the long exact sequence, we see that
\begin{align*}
...
\textbf{Big upshot.}
The collection of spaces $\{K(G, n)\}$ forms an $\Omega$-spectrum, which represents ordinary homology and cohomology with $G$-coefficients. Precisely, we have natural isomorphisms
$$\tilde{H}_q(X; G) \cong \colim_n \pi_{q+n}(X\wedge K(G, n)),$$
$$\tilde{H}^q(X; G)\cong [X, K(G,n)]_\ast.$$ 

This is discussed in \cite[Chapter 22]{may}. The idea of the proofs is to simply notice that the axioms for reduced homology and cohomology (which we have already checked in general) are satisfied for the expressions on the right hand side, including also the dimension axiom. Thus these are indeed isomorphic to ordinary homology and cohomology by the uniqueness of homology and cohomology theories that satisfy the dimension axiom.
